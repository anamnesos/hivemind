...
function nextRequestId() {
  requestCounter += 1;
  return `experiment-${Date.now()}-${requestCounter}`;
}
...
function handleWorkerMessage(msg) {
  ...
  const err = new Error(msg.error || 'experiment worker request failed');
  err.code = msg.code || 'EXPERIMENT_WORKER_ERROR';
  entry.reject(err);
}
...
function attachWorkerListeners(worker) {
  ...
  worker.on('error', (err) => {
    log.error('ExperimentWorker', `Worker process error: ${err.message}`);
  });
  ...
  worker.on('exit', (code, signal) => {
    ...
    rejectAllPending(new Error(`experiment worker exited (code=${code}, signal=${signal || 'none'})`));

    if (intentional) {
      log.info('ExperimentWorker', `Worker stopped (${signal || code || 'exit'})`);
    } else {
      log.error('ExperimentWorker', `Worker exited unexpectedly (code=${code}, signal=${signal || 'none'})`);
    }
  });
}
...
function sendRequestWithWorker(worker, type, payload = {}, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS) {
  if (!worker || typeof worker.send !== 'function') {
    return Promise.reject(new Error('experiment worker unavailable'));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      const entry = clearPendingRequest(reqId);
      if (!entry) return;
      const timeoutError = new Error(`experiment worker timeout (${type})`);
      timeoutError.code = 'EXPERIMENT_WORKER_TIMEOUT';
...
async function closeRuntime(options = {}) {
  ...
  try {
    ...
    log.warn('ExperimentWorker', `Close request failed: ${err.message}`);
  }
