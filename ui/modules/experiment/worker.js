 ...
 * Owns experiment runtime and executes operations off the main thread.
 */
...
function handleRequest(message) {
  ...
  try {
    if (msg.type === 'init') {
      const result = initializeExperimentRuntime(asObject(msg.options));
      sendResponse(reqId, true, result);
      return;
    }

    if (msg.type === 'op') {
      const result = executeExperimentOperation(
        msg.action,
...
function shutdown(exitCode = 0) {
  try {
    ...
    log.warn('ExperimentWorker', `Failed to close runtime during shutdown: ${err.message}`);
  }
...
process.on('uncaughtException', (err) => {
  log.error('ExperimentWorker', `Uncaught exception: ${err.message}`);
  shutdown(1);
});
process.on('unhandledRejection', (reason) => {
  const message = reason instanceof Error ? reason.message : String(reason);
  log.error('ExperimentWorker', `Unhandled rejection: ${message}`);
  shutdown(1);
});
