...
function nextRequestId() {
  requestCounter += 1;
  return `ledger-${Date.now()}-${requestCounter}`;
}
...
function handleWorkerMessage(msg) {
  ...
  const err = new Error(msg.error || 'evidence-ledger worker request failed');
  err.code = msg.code || 'EVIDENCE_LEDGER_WORKER_ERROR';
...
function attachWorkerListeners(worker) {
  ...
  worker.on('error', (err) => {
    log.error('EvidenceLedgerWorker', `Worker process error: ${err.message}`);
  });
  ...
  worker.on('exit', (code, signal) => {
    ...
    if (intentional) {
      ...
      log.error('EvidenceLedgerWorker', `Worker exited unexpectedly (code=${code}, signal=${signal || 'none'})`);
    }
  });
}
...
function ensureWorkerProcess() {
  ...
  const worker = fork(WORKER_PATH, [], {
    env: {
      ...process.env,
      HIVEMIND_EVIDENCE_LEDGER_WORKER: '1',
    },
  });
...
function sendRequestWithWorker(worker, type, payload = {}, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS) {
  if (!worker || typeof worker.send !== 'function') {
    return Promise.reject(new Error('evidence-ledger worker unavailable'));
  }
...
async function closeRuntime(options = {}) {
  ...
  try {
    ...
    log.warn('EvidenceLedgerWorker', `Close request failed: ${err.message}`);
  }
