...
const transcriptStore = require('../utils/transcript-store');
...
function loadSession(role, options = {}) {
  ...
  try {
    ...
    const entries = transcriptStore.readTranscript(role, { limit: limit * 2 });
...
function loadTimeRangeSession(startTime, endTime) {
  ...
  for (const role of roles) {
    const entries = transcriptStore.readTranscript(role, { limit: 5000 });
