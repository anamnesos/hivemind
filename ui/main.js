...
const DEFAULT_SETTINGS = {
  ...
  ptyStuckDetection: false,  // DISABLED: Bug - misinterprets thinking as stuck (Session 54)
...
function initRecoveryManager() {
  ...
  recoveryManager = createRecoveryManager({
    ...
    requestUnstick: (paneId, info = {}) => {
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('unstick-pane', {
          paneId: String(paneId),
          source: 'recovery',
          ...info,
        });
...
async function initDaemonClient() {
  ...
  daemonClient.on('data', (paneId, data) => {
    ...
    recoveryManager?.recordPtyOutput?.(paneId, data);
