...
6. **ALWAYS message Architect on startup** (even if no issues):
   ```
   echo "(ANALYST #1): Analyst online. [status summary]" > "D:\projects\hivemind\workspace\triggers\architect.txt"
   ```
...
**NOTE:** Your trigger file is `analyst.txt`. Other agents message you by writing to `D:\projects\hivemind\workspace\triggers\analyst.txt`.

### CRITICAL: USE ABSOLUTE PATHS

Your working directory is `D:\projects\hivemind\workspace\instances\investigator\`. Relative paths will resolve WRONG and create ghost files.

**ALWAYS use absolute paths like this:**
```
echo "(ANALYST #N): message" > "D:\projects\hivemind\workspace\triggers\architect.txt"
```
