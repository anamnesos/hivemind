...
SquidRun agents can commit and push code, but the GitHub workflow layer is entirely manual:
...
- CI status (GitHub Actions) is invisible inside SquidRun
...
Bring GitHub PR, issue, and CI workflows inside SquidRun so agents can:
...
4. Trigger code review workflows from within SquidRun
...
**Methods:**
```
createPR({ title, body, base, head, draft }) → { number, url, html_url }
updatePR(number, { title, body, state }) → { number, url }
getPR(number) → { number, title, body, state, mergeable, checks }
listPRs({ state, head, base }) → [{ number, title, state, ... }]
mergePR(number, { method }) → { merged, message }

createIssue({ title, body, labels, assignees }) → { number, url }
getIssue(number) → { number, title, body, state, labels }
listIssues({ state, labels }) → [{ number, title, state, ... }]
closeIssue(number) → { number, state }
addIssueComment(number, body) → { id }

getChecks({ ref }) → [{ name, status, conclusion, url }]
getWorkflowRuns({ branch, status }) → [{ id, name, status, conclusion }]

getRepo() → { owner, repo, default_branch, url }
```
...
**Channels:**
```
github:create-pr       → createPR(payload)
github:update-pr       → updatePR(number, payload)
github:get-pr          → getPR(number)
github:list-prs        → listPRs(filters)
github:merge-pr        → mergePR(number, options)

github:create-issue    → createIssue(payload)
github:get-issue       → getIssue(number)
github:list-issues     → listIssues(filters)
github:close-issue     → closeIssue(number)
github:comment-issue   → addIssueComment(number, body)

github:get-checks      → getChecks(ref)
github:get-runs        → getWorkflowRuns(filters)

github:get-repo        → getRepo()
github:auth-status     → check if gh CLI is authenticated
```
...
### Layer 4: WebSocket Routing (`ui/modules/main/squidrun-app.js`)
...
---
🤖 Generated by SquidRun Architect
```
...
- WebSocket routing in squidrun-app.js
...
- `github-service.test.js` — mock `exec` for `gh` CLI calls
- `github-handlers.test.js` — IPC harness tests
- `hm-github.test.js` — CLI argument parsing + WebSocket mock
